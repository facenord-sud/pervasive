class MultiplySensors
  ...
  attr_accessor :tank_sensors, :room_sensors, :ext_temperature
  ...
  def to_hash
    tank_sensors.merge(room_sensors).merge(ext_temperature)
  end

  def generate(energy, temp_int, weather)
    @tank_sensors = generate_tank_sensors energy
    @room_sensors = generate_room_sensors temp_int
    @ext_temperature = generate_ext_temperature weather
  end
